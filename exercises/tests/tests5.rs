...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: This block of code dereferences the given address, assuming it points to a valid
    // mutable reference to a `u32` value. It then performs modification on the value pointed
    // to by the address. This is marked as unsafe because it is the caller's responsibility
    // to ensure that the given address is valid and properly aligned, and that the data pointed
    // to by the address is indeed a mutable `u32` value. Failure to uphold these requirements
